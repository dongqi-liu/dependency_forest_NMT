...
from iwslt2015 import IWSLT2015
from typing import Iterable, List
from no_type import dependency_tree_without_type
import notype_encoder as dep
import datetime
import sacrebleu
...
import random
import spacy
mylog = open('iwslt_notype_en_zh_recode.log', mode = 'a',encoding='utf-8')
...
for ln in [SRC_LANGUAGE, TGT_LANGUAGE]:
    ...
    train_iter = IWSLT2015(split='test', language_pair=(SRC_LANGUAGE, TGT_LANGUAGE))
...
def save_model(score,best_score,model):
    if score > best_score:
        ...
        torch.save(checkpoint, 'iwslt_notype_en_zh_ckpt_best.pt')
    return best_score
...
def get_metric(Tokenizer,TEXT,SRC_LANGUAGE,TGT_LANGUAGE):
    ...
    final = []
    date_iter = IWSLT2015(split=TEXT, language_pair=(SRC_LANGUAGE, TGT_LANGUAGE))
    ...
    # ribes = corpus_ribes(references_corpus_token_list, candidate_corpus_token_list)
...
scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps = 0, num_training_steps = 300)
...
def collate_fn(batch):
    ...
    for src_sample, tgt_sample in batch:
        if len(src_sample.rstrip("\n")) >0 and len(tgt_sample.rstrip("\n")) >0:
    ...
    dependency_forest_set = dependency_tree_without_type(parser,src_batch_list,src_sample_list)
...
def train_epoch(model, optimizer):
    model.train()
    losses = 0
    train_iter = IWSLT2015(split='train', language_pair=(SRC_LANGUAGE, TGT_LANGUAGE))
...
def evaluate(model):
    with torch.no_grad():
        ...
        val_iter = IWSLT2015(split='valid', language_pair=(SRC_LANGUAGE, TGT_LANGUAGE))
