...
import random
import spacy
mylog = open('multi30k_baseline_en_de_recode.log', mode = 'a',encoding='utf-8')
...
class Seq2SeqTransformer(nn.Module):
    def __init__(self,
        ...
        self.dependency_forest = nn.TransformerEncoderLayer(d_model=emb_size, nhead=nhead)
    ...
    def forward(self,
        ...
        dependency_forest_outs = self.dependency_forest(src_emb)
...
def get_metric(Tokenizer,TEXT,SRC_LANGUAGE,TGT_LANGUAGE):
    ...
    final = []
    date_iter = Multi30k(split=TEXT, language_pair=(SRC_LANGUAGE, TGT_LANGUAGE))
    ...
    print("RIBES score on the " + TEXT + " set is " + str(ribes),file=mylog)
    print("TER score on the " + TEXT + " set is " + str(ter.corpus_score(candidate_corpus_list, final)),file=mylog)
    return bleu4
...
def save_model(score,best_score,model):
    if score > best_score:
        ...
        torch.save(checkpoint, 'multi30k_baseline_en_de_ckpt_best.pt')
    return best_score
